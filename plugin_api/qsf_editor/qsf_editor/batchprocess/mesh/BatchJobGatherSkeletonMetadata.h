...
namespace qsf
{
	namespace editor
	{
		...
		/**
		*  @brief
		*    Batch job for gathering skeleton metadata
		*
		*  @note
		*    - Only exists to update assets which had been imported before the related feature had been implemented
		*/
		class QSF_EDITOR_API_EXPORT BatchJobGatherSkeletonMetadata : public BatchJob
		{
		...
		public:
			...
			*    Constructor
			*/
			explicit BatchJobGatherSkeletonMetadata(BatchJobManager* manager);

			/**
			*  @brief
			*    Destructor
			*/
			virtual ~BatchJobGatherSkeletonMetadata();
		...
		//[-------------------------------------------------------]
		private:
			std::vector<GlobalAssetId>			mGlobalSkeletonAssetIds;
...
//[ CAMP reflection system                                ]
//[-------------------------------------------------------]
QSF_CAMP_TYPE_NONCOPYABLE(qsf::editor::BatchJobGatherSkeletonMetadata)
