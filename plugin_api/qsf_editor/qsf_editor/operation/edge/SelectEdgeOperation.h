...
namespace qsf
{
	namespace editor
	{
		...
		/**
		*  @brief
		*    Select edge component edge operation class
		*
		*  @note
		...
		*/
		class SelectEdgeOperation : public EdgeSelectionOperation
		{
		...
		public:
			...
			*  @param[in] edgeId
			*    ID of the edge to select
			*/
			SelectEdgeOperation(uint64 entityId, uint32 componentId, unsigned int edgeId);

			/**
			*  @brief
			*    Destructor
			*/
			virtual ~SelectEdgeOperation();
		...
		//[-------------------------------------------------------]
		private:
			unsigned int				mEdgeId;			///< Edge ID to select
