...
namespace qsf
{
	namespace editor
	{
		...
		/**
		*  @brief
		*    Modify raw nodes edit mode class. Differs from the ModifyNodesEditMode that here is no
		*    logic for special cases. Each node is isolated for editing and can be modified without
		*    restrictions (e.g. complete TRS modification)
		*/
		class ModifyRawNodesEditMode : public NodesEditMode
		{
		...
		//[-------------------------------------------------------]
		public:
			static const uint32 PLUGINABLE_ID;	///< "qsf::editor::ModifyRawNodesEditMode" unique pluginable edit mode ID
		...
		public:
			...
			*/
			explicit ModifyRawNodesEditMode(EditModeManager* editModeManager);

			/**
			*  @brief
			*    Destructor
			*/
			virtual ~ModifyRawNodesEditMode();
		...
		//[-------------------------------------------------------]
		//[ Protected virtual qsf::editor::ModifyRawNodesEditMode methods ]
		//[-------------------------------------------------------]
		protected:
			/**
			*  @brief
		...
		//[-------------------------------------------------------]
		//[ Protected virtual qsf::editor::ModifyRawNodesEditMode Qt slots (MOC) ]
		//[-------------------------------------------------------]
		protected Q_SLOTS:
...
//[ CAMP reflection system                                ]
//[-------------------------------------------------------]
QSF_CAMP_TYPE_NONCOPYABLE(qsf::editor::ModifyRawNodesEditMode)
