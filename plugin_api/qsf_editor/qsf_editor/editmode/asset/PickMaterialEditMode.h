...
namespace qsf
{
	namespace editor
	{
		...
		/**
		*  @brief
		*    Pick material edit mode class
		*/
		class PickMaterialEditMode : public EditMode
		{
		...
		//[-------------------------------------------------------]
		public:
			static const uint32 PLUGINABLE_ID;	///< "qsf::editor::PickMaterialEditMode" unique pluginable edit mode ID
		...
		Q_SIGNALS:
			/**
			*  @brief
			*    Qt signal emitted after a material has been picked
			*
			*  @param[in] globalAssetId
			*    The global asset ID of the picked material
			*/
			void materialPicked(GlobalAssetId globalAssetId);
		...
		public:
			...
			*/
			explicit PickMaterialEditMode(EditModeManager* editModeManager);

			/**
			*  @brief
			*    Destructor
			*/
			virtual ~PickMaterialEditMode();
...
//[ CAMP reflection system                                ]
//[-------------------------------------------------------]
QSF_CAMP_TYPE_NONCOPYABLE(qsf::editor::PickMaterialEditMode)
