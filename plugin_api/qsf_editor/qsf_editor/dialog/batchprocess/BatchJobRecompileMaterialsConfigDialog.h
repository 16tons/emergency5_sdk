...
namespace Ui
{
	class BatchJobRecompileMaterialsConfigDialog;
}
...
namespace qsf
{
	namespace editor
	{
		...
		*/
		class BatchJobRecompileMaterialsConfigDialog : public Dialog
		{
		...
		//[-------------------------------------------------------]
		public:
			static const uint32 PLUGINABLE_ID;	///< "qsf::editor::BatchJobRecompileMaterialsConfigDialog" unique pluginable dialog ID
		...
		public:
			...
			*/
			BatchJobRecompileMaterialsConfigDialog(DialogManager* dialogManager, QWidget* qWidgetParent = nullptr);

			/**
			*  @brief
			*    Destructor
			*/
			virtual ~BatchJobRecompileMaterialsConfigDialog();
			...
			void setForceRecompile(bool state);
			bool isForceRecompile() const;
		...
		private Q_SLOTS:
			...
			void onClickedSelectOutdated(bool checked);
		...
		//[-------------------------------------------------------]
		private:
			Ui::BatchJobRecompileMaterialsConfigDialog*	mUiDialog;	///< UI dialog instance, can be a null pointer, we have to destroy the instance in case we no longer need it
...
//[ CAMP reflection system                                ]
//[-------------------------------------------------------]
QSF_CAMP_TYPE_NONCOPYABLE(qsf::editor::BatchJobRecompileMaterialsConfigDialog)
