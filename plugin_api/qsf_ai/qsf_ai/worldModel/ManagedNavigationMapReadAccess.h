...
namespace qsf
{
	...
	{
		/**
		* A managed navigation map read access is a thin wrapper around a world model that provides read access.
		* This contains a lock that denies concurrent reads and writes.
		...
		*/
		class QSF_AI_API_EXPORT ManagedNavigationMapReadAccess
		{
		public:
			...
			ManagedNavigationMapReadAccess(const WorldModel& navigationMap, boost::shared_mutex& accessMutex);
			...
			ManagedNavigationMapReadAccess(const WorldModel& navigationMap, boost::shared_mutex& accessMutex, TryLock onlyTryToLock);

			operator const WorldModel&();
			const WorldModel& operator *();
			const WorldModel& get();
		...
		private:
			boost::shared_lock<boost::shared_mutex> mAccessLock;
			const WorldModel& mNavigationMap;
		};
	}
}
