...
namespace qsf
{
	...
	*/
	class QSF_API_EXPORT Metaclasses : public boost::noncopyable
	{
	...
	protected:
		...
		*/
		...
		/**
		*  @brief
		*    Add a class forwarding between registered classes, leading to the old class ID to reference the new class
		*
		*  @param[in] oldClassId
		*    Old class identifier, which is the hash of the class name (e.g. "qsf::MetadataComponent")
		*  @param[in] newClassId
		*    New class identifier
		*/
		void addCampClassForwarding(const StringHash& oldClassId, const StringHash& newClassId);

		/**
		*  @brief
		*    Add a class as deprecated, i.e. there won't be errors when this class is not found
		*
		*  @param[in] classId
		*    Class identifier, which is the hash of the class name (e.g. "qsf::MetadataComponent")
		*/
		void addDeprecatedCampClass(const StringHash& classId);

		/**
		*  @brief
		*    Add a class as deprecated and renamed, together with the new class name
		*
		*  @param[in] classId
		*    Old class identifier, which is the hash of the class name (e.g. "qsf::MetadataComponent")
		*  @param[in] newClassId
		*    New class identifier
		*/
		void addDeprecatedCampClass(const StringHash& classId, const StringHash& newClassId);
		...
		*/
		void addDeprecatedCampClassProperty(const StringHash& classId, const StringHash& propertyId);
