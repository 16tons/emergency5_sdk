	...
	namespace multiplayer
	{
		...
		bool HealthData::prepareForUpdate(const qsf::Entity& targetEntity, const qsf::Clock& clock)
		{
			if (mComponent.valid())
			{
				...
				const bool contaminatedChanged = mComponent->isContaminated() != mIsContaminated;
				if (contaminatedChanged)
				{
					mIsContaminated = mComponent->isContaminated();
				}
				...
				return mInjuryIdChanged || mHealthChanged || mLifeChanged || mDeathChanged || mStateChanged || contaminatedChanged || mTreatmentActiveChanged || mDiagnosisActiveChanged || mDiagnosisCompleteChanged;
			}
		...
		void HealthData::updateData(qsf::game::BitStream& bitStream, bool force)
		{
			...
			bitStream.write(mIsContaminated);
		...
		void HealthData::setData(const qsf::game::BitStream& bitStream, const int32 receivedHostTickCount)
		{
			...
			bitStream.read(entry.mIsContaminated);
			...
			if (!mHistoryList.empty() && mComponent.valid())
			{
				...
				while (iterator != mHistoryList.end())
				{
					...
					if (entry.tickCount == clock.getSignalCounter())
					{
						...
						if (!entry.mIsContaminated && mComponent->isContaminated())
						{
							// The data has changed -> apply it
							mComponent->decontaminate();
						}
